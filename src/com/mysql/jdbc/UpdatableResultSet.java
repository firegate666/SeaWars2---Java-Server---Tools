...
public class UpdatableResultSet extends ResultSet {
        ...
        try {
            ...
            if (numKeys == 1) {
                ...
                String currentVal = ((characterEncoding == null)
                    ...
                    : new String((byte[]) this.thisRow[index], characterEncoding));
        ...
        try {
            ...
            if (rs.next()) {
                ...
                throw new SQLException(Messages.getString(
                        "UpdatableResultSet.12"), //$NON-NLS-1$
    ...
    public synchronized void updateBoolean(int columnIndex, boolean x)
        ...
        if (!this.onInsertRow) {
            ...
            this.thisRow[columnIndex - 1] = this.inserter.getBytesRepresentation(columnIndex -
                    1);
        }
    }
    ...
    synchronized void syncUpdate() throws SQLException {
        ...
        if (numKeys == 1) {
            ...
            for (int i = 0; i < numKeys; i++) {
                byte[] currentVal = (byte[]) this.thisRow[((Integer) this.primaryKeyIndicies.get(i)).intValue()];
    ...
    private void checkUpdatability() throws SQLException {
        ...
        if ((this.catalog == null) || (this.catalog.length() == 0)) {
            ...
            if ((this.catalog == null) || (this.catalog.length() == 0)) {
                throw new SQLException(Messages.getString(
                        "UpdatableResultSet.43") //$NON-NLS-1$
                , SQLError.SQL_STATE_ILLEGAL_ARGUMENT); //$NON-NLS-1$ //$NON-NLS-2$
            }
        }
